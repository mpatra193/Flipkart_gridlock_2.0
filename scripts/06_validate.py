...
def main():
    ...
    sensitivity_kappa()
    duration_bands()
    out = config.ROOT / "docs" / "VALIDATION.md"
    out.parent.mkdir(parents=True, exist_ok=True)
    with open(out, "w", encoding="utf-8") as fh:
        fh.write("# ASTRA — Validation & Calibration Appendix\n\n")
        fh.write("Auto-generated by `scripts/06_validate.py` from the event dataset.\n\n```\n")
        fh.write("\n".join(LINES).strip() + "\n```\n")
