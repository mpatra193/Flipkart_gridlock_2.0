...
astra/
├── backend/
│   ├── main.py                      # FastAPI app entrypoint, loads all in-memory state on startup
│   ├── routes/
│   │   └── api.py                   # /api/predict, /api/junctions, /api/similar, etc.
│   ├── engines/
│   │   ├── duration_model.py        # LightGBM load + inference
│   │   ├── esi.py                   # Event Severity Index formula
│   │   ├── impact_radius.py         # congestion-spread formula
│   │   ├── spillover_graph.py       # NetworkX graph build + BFS propagation
│   │   ├── similar_events.py        # weighted k-NN over historical events
│   │   ├── diversion.py             # corridor scoring + historical cascade analysis
│   │   └── resource_planner.py      # officer/barricade/patrol-vehicle formulas
│   ├── data/
│   │   ├── raw/events.csv
│   │   ├── junction_risk.csv
│   │   ├── zone_risk.csv
│   │   ├── corridor_risk.csv
│   │   └── junction_road_distances.csv
│   ├── models/
│   │   └── duration_lgbm.pkl
│   ├── notebooks/
│   │   └── eda_and_training.ipynb   # data audit + model training notebook
│   └── requirements.txt
│
├── frontend/
│   ├── src/
...
│   │   │   └── usePrediction.ts
│   │   ├── types/
│   │   │   └── astra.ts             # EventInput, PredictionResult, etc.
│   │   └── App.tsx
│   ├── package.json
│   └── vite.config.ts
│
├── impact_radius.py                 # standalone prototype — current implementation (see §15)
├── impact_radius_demo.html          # generated Folium map output from the prototype
├── DESIGN_SPEC.md                   # original full design spec — every formula, table, derivation
└── README.md
```
...
Verify: `http://localhost:8000/docs` should show the interactive API explorer.
...
### Running the current prototype (no backend/frontend yet — see §15)

```bash
cd astra
pip install pandas numpy folium
python impact_radius.py
```

This runs the impact-radius + affected-junctions engine against the raw dataset and writes `impact_radius_demo.html` — open it in a browser to see the dark-themed map with the event marker, impact circle, and colour-coded junction risk dots.
...
4. **Provision the backend:**
   ...
   git clone <repo-url> && cd astra/backend
...
This repo is at the **prototype stage** — the design above is the target architecture; what's actually built and runnable today is:

- ✅ [`impact_radius.py`](impact_radius.py) — the full Impact Radius Engine (duration → base radius → closure/peak/cause multipliers), `find_affected_junctions()` with HIGH/MEDIUM/LOW ring classification, and a Folium dark-map visualizer ([`impact_radius_demo.html`](impact_radius_demo.html) is a generated sample output).
- ⏳ Duration prediction model (LightGBM), ESI scoring, spillover graph, similar-event engine, diversion engine, resource planner, FastAPI backend, and React frontend are designed in full (Sections 5–10 above, and in exhaustive detail in [`DESIGN_SPEC.md`](DESIGN_SPEC.md)) but not yet implemented in this repo.

This README documents the complete intended system so the architecture, ML reasoning, and every formula are reviewable independent of build progress.
